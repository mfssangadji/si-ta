...
@section('content')
		...
		@if($sesi->status == 0)
			@if(Auth::guard('dosen'))
				<form method="post" action="{{url('bimbingan-dosen/'.Request::segment(2).'/'.Request::segment(3).'/konsultasi/confirm')}}" style="display:inline;">
					@csrf
						...
						<div class="block_content">
							...
							<p class="excerpt">
								...
								@if($k->lampiran != "")
									...
									<a target="_blank" href="{{url('bimbingan-dosen/'.Request::segment(2).'/'.Request::segment(3).'/konsultasi/'.$k->id)}}" style='color:#FFF'>Download Lampiran</a>
									</span>
								@endif
							</p>
						</div>
		...
		@if($sesi->status == 0)
			<form id="demo-form2" method="post" action="{{url('bimbingan-dosen/'.Request::segment(2).'/'.Request::segment(3).'/konsultasi')}}" enctype="multipart/form-data" data-parsley-validate class="form-horizontal form-label-left">
				@csrf
